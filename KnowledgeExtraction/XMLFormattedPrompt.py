...
def MergeParagraphs(text):
    ...
    return '\n\n'.join([i.replace('\n', '').replace('. . . ', '').replace('. . . ', '').replace('. . . ', '') for i in
                        processed_lines]).replace('. . . ', '').replace('. . . ', '').replace('. . . ', '').replace(
        '. . . ', '').replace('. . . ', '')
def GetRefineContents(Contents):
    for i in range(10):
        Contents = Contents.replace('. . . ', '')
    Contents = Contents.replace('ﬀ', 'ff').replace('', 'fi').replace('ﬁ', 'fi').replace('ﬂ', 'fl').replace('ﬃ',
                                                                                                            'ffi').replace(
        'ﬄ', 'ffl').replace('ﬅ', 'ft').replace('ﬆ', 'st').split('\n')
...
def split_text(text, max_tokens):
    ...
    return chunks
def GetDataList(Folder, MaxToken=25000):
    os.chdir(Folder)
        ...
        for chunk_index, doc_chunk in enumerate(doc_chunks):
            for i, j in enumerate(AllPrompt):
                full_prompt = (FormattedPromptHead + '\n' + doc_chunk.strip().replace('\n\n', '\n') +
                               '\n' + FormattedPromptMiddle + '\n' + j.strip().replace('\n\n', '\n') +
                               '\n' + FormattedPromptEnd)
                output_filename = f'Prompt{os.sep}Prompt{folder}.PART{i}.CHUNK{chunk_index}'
