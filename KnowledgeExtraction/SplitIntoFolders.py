    ...
    for PART in PARTS:
        ...
        for Name in [i for i in os.listdir('.') if i.startswith('10.') and i.endswith('.txt')]:
            ...
            try:
                ...
                OutOfRange = [i for i in Questions.keys() if i not in [str(j) for j in [
                    *range(int(PART.removeprefix('PART')) * chunk_size + 1,
                ...
                for i in Questions.keys():
                    if i not in OutOfRange:
                        ...
                        Answer = {j: Questions[i].findall(j)[0].text if Questions[i].findall(j) else '' for j in
                                  ['Quotes', 'English', 'Chinese']}
                        Answer = {k: (v.strip() if v else '') for k, v in Answer.items()}
                        Answer.update({'Doi': DOI})
                        NoChineseAnswer = {j: Questions[i].findall(j)[0].text if Questions[i].findall(j) else '' for j
                                           in ['Quotes', 'English', ]}
                        ...
                        [open(f'{Folder}{os.sep}{j}.txt', 'a', encoding="utf-8").write(Answer[j] + '\n\n') for j in
                         ['Quotes', 'English', 'Chinese']]
                        open(f'{Folder}{os.sep}All.txt', 'a', encoding="utf-8").write(
        ...
        if ErrorList:
            RemoveCommands.append(
                f"for i in {{{','.join(ErrorList)}}};do rm {PART}{os.sep}`readlink {PART}{os.sep}$i`;done")
