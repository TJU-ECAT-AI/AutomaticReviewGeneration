                ...
                while True:
                    ...
                    try:
                        ...
                        if 'Outlines' in response:
                            content = '<?xml version="1.0" encoding="UTF-8"?>\n<Outlines>' + response.split('<Outlines>',1)[1].rsplit('</Outlines>',1)[0] + '</Outlines>'
                            try:
...
def ReshapeOutlines(text):
    ...
    try:
        ...
        if '<Outlines>' not in text:
            return text
        content = '<?xml version="1.0" encoding="UTF-8"?>\n<Outlines>' + text.split('<Outlines>',1)[1].rsplit('</Outlines>',1)[0] + '</Outlines>'
        root = ET.fromstring(content)       
        formatted_lines = []
        global outline
...
<file-attachment-contents filename='"""
    ...
    if not os.path.exists(f"AllQuestionsFromReview{os.sep}QuestionsFromReview_0.txt"):
        Folders = [i for i in os.listdir('.') if (i.startswith('10.') and not i.endswith('.txt')) or i=='DirectGeneration' ]
        Folders.sort()
        os.makedirs('AllQuestionsFromReview', exist_ok=True)
        AllQuestionsFromReview = ''
        for folder in Folders:
            AllQuestionsFromReview += '\n\n\n'.join(
                [open(f'{folder}{os.sep}QuestionsFromReview_{Time}.txt', 'r', encoding='UTF8').read() for Time in
                 range(REPEAT)])
...
<file-attachment-contents filename='"""
    ...
    if not os.path.exists(f"PaperQuestionsFromParagraphQuestionsForReview{os.sep}QuestionsFromReview_0.txt"):
        ...
        while len([i for i in Questions.splitlines() if i.strip()]) != len(
                [i for i in ReviewOutline.splitlines() if i.strip()]):
