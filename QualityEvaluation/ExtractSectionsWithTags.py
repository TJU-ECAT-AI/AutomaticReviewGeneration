...
def Main(Folder,STDOUT):
    ...
    current_directory = "."
    txt_files = [int(file.strip('.txt').replace('BestParagraph','')) for file in os.listdir(current_directory) if file.endswith('.txt') and file.startswith('BestParagraph') and 'draft' not in file and '_' not in file]
    txt_files.sort()
...
if __name__ == "__main__":
    Main(f'Answer{os.sep}Paragraph',sys.stdout)
