    ...
    for category, subcategories in data.items():
        if category != 'Overall Evaluation':
            for subcategory, details in subcategories.items():
                row = {
                    'Category': category,
                    'Subcategory': subcategory if not subcategory.startswith('Feasibility') else 'Feasibility of Proposed Suggestions',
...
def create_analysis_df():
    ...
    all_files= [f for f in all_files if re.match(r'Comparison\d_Paragraph\d+_\d+@Paragraph\d+_\d+\.txt$', f)]
    all_data = []
    ...
    df['Subcategory'] = df['Subcategory'].replace('Feasibility of Policy Suggestions', 'Feasibility of Proposed Suggestions')
...
def calculate_icc(df, subcat):
    ...
    icc = pg.intraclass_corr(data=icc_data, targets='Pair', raters='Comparison', ratings='Score_Diff',nan_policy='omit')
...
merged_df = pd.merge(icc_summary, transitivity_df[['Subcategory', 'Transitivity_consistency_ratio']], on='Subcategory', how='outer')
