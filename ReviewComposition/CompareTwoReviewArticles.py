...
def extract_sections_with_tags(content):
    ...
    reference_matches = reference_matches if reference_matches else re.findall(r"<Reference>(.*?)</Reference>", content,
                                                                               re.DOTALL)
    return '\n'.join(english_matches), '\n'.join(chinese_matches), '\n'.join(reference_matches)
def GetResponseConcurrent(prompts, Threads, STDOUT, FunctionClaudeAPI, FunctionOpenAIAPI):
    prompt_queue = Queue()
    for item in prompts:
        prompt_queue.put(item)
    ClaudeAPISemaphore = [Semaphore(value=1) for _ in FunctionClaudeAPI]
    OpenAIAPISemaphore = [Semaphore(value=3) for _ in FunctionOpenAIAPI]
            ...
            while True:
                ...
                try:
                        ...
                        for semaphore, GetResponseFunction in zip(OpenAIAPISemaphore, FunctionOpenAIAPI.values()):
                            if semaphore.acquire(blocking=False):
                                ...
                                if response:
                                    break
                    if not response:
                        raise Exception("Failed to get response from any API")
                    try:
                        json_response = json.loads('{'+response.split('{',1)[1].rsplit('}',1)[0]+'}')
                        if not all(key in json_response for key in
                                   ["Content Quality", "Structure and Organization", "Academic Standards",
                                    "Overall Evaluation"]):
                            raise ValueError("Invalid JSON structure")
                    except (json.JSONDecodeError, ValueError) as e:
                        open(os.path.join(f'CompareParagraph', f'Comparison{Repeat}_{part}_{TRY}_error.txt'), 'w',
                             encoding='UTF8').write(f"Error: {str(e)}\n\nResponse:\n{response}\n")
                        TRY += 1
                        if TRY >= 3:
                            raise Exception(f"Failed to get valid response after {TRY} attempts")
                        continue
                    with open(os.path.join(f'CompareParagraph', f'Comparison{Repeat}_{part}.txt'), 'w',
                    ...
                    open('Exceptionlog', 'a').write(
                        f"{i}\t{folder}\t{part}\t{time.strftime('%Y-%m-%d_%H.%M.%S', time.localtime())}\t{error_msg}\n")
                    TRY += 1
                    if TRY >= 3:
                        break
                    time.sleep(5)
    ...
    for thread in threads:
        thread.join()
    progress_bar.close()
def Main(Folder, Threads, REPEAT, STDOUT, FunctionClaudeAPI, FunctionOpenAIAPI):
    old_stdout = sys.stdout
    sys.stdout = STDOUT
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    questions_file_path = os.path.join(base_dir, 'Temp', 'ParagraphQuestionsForReview.txt')
    os.chdir(Folder)
    with open(questions_file_path, 'r', encoding='UTF8') as f:
        ParagraphQuestionsForReview = [i.strip() for i in f if i.strip()]
    os.makedirs('CompareParagraph', exist_ok=True)
    paragraph_files = [f for f in os.listdir() if re.match(r"^Paragraph\d+_\d+\.txt$", f)]
    ...
    for i, (file1, file2) in enumerate(comparisons):
        with open(file1, 'r', encoding='UTF8') as f1, open(file2, 'r', encoding='UTF8') as f2:
            ...
            content1 = "\n".join(re.findall(r"<English>(.*?)</English>", content1, re.DOTALL)).strip() if "\n".join(
                re.findall(r"<English>(.*?)</English>", content1, re.DOTALL)).strip() else content1
            content2 = f2.read().strip().replace('※※※', '')
            content2 = "\n".join(re.findall(r"<English>(.*?)</English>", content2, re.DOTALL)).strip() if "\n".join(
                re.findall(r"<English>(.*?)</English>", content2, re.DOTALL)).strip() else content2
        question_index = int(re.search(r"Paragraph(\d+)_", file1).group(1)) - 1
        prompt = replace_with_html_entities(
            HEAD + ParagraphQuestionsForReview[question_index] + MIDDLE0 + content1 + MIDDLE1 + content2 + END)
    ...
    GetResponseConcurrent(prompts, Threads, STDOUT, FunctionClaudeAPI, FunctionOpenAIAPI)
    os.chdir('..')
    sys.stdout = old_stdout
if __name__ == "__main__":
    target_dir = os.path.join('..', 'Temp', 'ReviewCompositionWorkDir', 'Paragraph')
    Main(target_dir, 26, 5, sys.stdout)
