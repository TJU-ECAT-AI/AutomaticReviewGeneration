...
def process_file(file_path):
    ...
    df['File'] = file_name
    return df
def create_analysis_df(target_dir):
    file_pattern = "Comparison*_Paragraph*_*@Paragraph*_*.txt"
    full_pattern = os.path.join(target_dir, file_pattern)
    all_files = glob.glob(full_pattern)
    all_files = [f for f in all_files if
                 re.match(r'Comparison\d+_Paragraph\d+_\d+@Paragraph\d+_\d+\.txt$', os.path.basename(f))]
    all_data = []
    ...
    Subcategorys = df['Subcategory'].unique()
...
def calculate_icc(df, subcat):
    try:
        ...
        df['Score_Diff'] = (df['Review A Score'].fillna(0) - df['Review B Score'].fillna(0)) * df['Sign']
        df['Comparison'] = (df['Comparison'].map(int) + 1) * df['Sign']
        icc_data = df[['Pair', 'Comparison', 'Score_Diff']]
        if len(icc_data) < 5 or len(icc_data['Pair'].unique()) < 5:
            print(f"Warning: Insufficient data points for {subcat}")
            return pd.DataFrame({
                'Type': ['ICC2'],
                'ICC': [float('nan')],
                'CI95%': [float('nan')],
        ...
        print(f"Error calculating ICC for {subcat}: {str(e)}")
        return pd.DataFrame({
            'Type': ['ICC2'],
            'ICC': [float('nan')],
            'CI95%': [float('nan')],
...
def main(folder_path):
    ...
    try:
        df, Subcategorys = create_analysis_df(folder_path)
    except ValueError as e:
        print(f"Error: {str(e)}")
        print("Please check if the folder contains the correct files and if you have the necessary permissions.")
        return
    df['Review A Score'] = df['Review A Score'].fillna(0)
    df['Review B Score'] = df['Review B Score'].fillna(0)
    df.to_csv(os.path.join(output_folder, 'RawResult.csv'))
    ...
    output_df['Score'] = output_df['Score'] / 2.7
    output_df.to_csv(os.path.join(output_folder, 'pagerank_results_balanced.csv'), index=False)
    data_show = output_df.groupby('Author')['Score'].sum().sort_values()
    print('Authors and scores:')
    print(data_show)
    author_scores_df = data_show.reset_index()
    author_scores_df.columns = ['Author', 'Total Score']
    author_scores_df = author_scores_df.sort_values('Total Score', ascending=False)
    author_scores_path = os.path.join(output_folder, 'author_scores.csv')
    author_scores_df.to_csv(author_scores_path, index=False)
    print('\nBest author and score:')
    print(pd.concat([data_show.tail(1)]))
    print("\nResults have been saved to 'author_scores.csv'")
    ...
    all_data.to_csv(os.path.join(output_folder, 'bland_altman_data.csv'), index=False)
    ...
    for i in range(idx + 1, len(axes)):
        fig.delaxes(axes[i])
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'Bland-Altman.svg'))
    icc_results = []
    ...
    merged_df = pd.merge(icc_summary, transitivity_df[['Subcategory', 'Transitivity_consistency_ratio']],
                         on='Subcategory',
                         how='outer')
    ...
    merged_df.to_csv(os.path.join(output_folder, 'reliability_analysis.csv'), index=False)
...
def organize_files(path):
    pattern = re.compile(r'Comparison\d+_Paragraph(\d+)_\d+@Paragraph\d+_\d+\.txt')
    folders = []
    for filename in os.listdir(path):
        match = pattern.match(filename)
        if match:
            category = match.group(1)
            folder_name = f'Paragraph{category}'
    ...
    return folders
def Main2(path):
    dirs = organize_files(path)
    for dir in dirs:
        main(path + '/' + dir)
if __name__ == "__main__":
    path = '../Temp/ReviewCompositionWorkDir/Paragraph/CompareParagraph'
    Main2(path)
